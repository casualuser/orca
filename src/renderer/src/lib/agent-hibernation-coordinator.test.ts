...
describe('agent sleep coordinator', () => {
  ...
  it('revalidates fresh state before shutdown', async () => {
    ...
    await vi.advanceTimersByTimeAsync(1000)
    // Why: returning to the tab between the plan and the confirm is the eligibility change
    // this must observe — the tree being worked in is no longer skipped wholesale (#16211).
